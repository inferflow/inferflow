...
///////////////////////////////////////////////////////////////////////////
// Added codes for Inferflow

void ggml_clear_context(struct ggml_context * ctx) {
    ctx->n_objects = 0;
    ...
    ctx->scratch_save.offs = 0;
    ctx->scratch_save.size = 0;
    ctx->scratch_save.data = 0;
}

void ggml_tensor_array_init(struct ggml_tensor_array *tensor_array)
{
    tensor_array->size = 0;
}

// End of added codes for Inferflow
///////////////////////////////////////////////////////////////////////////
...
{
    ggml_graph_compute_ext(ctx, cgraph, NULL);
}

void ggml_graph_compute_ext(struct ggml_context * ctx, struct ggml_cgraph * cgraph,
    ggml_tensor_array * tensor_array)
{
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (tensor_array != NULL && node->id != 0 && (node->id < 20000 || node->id >= 990000 || node->id % 10000 == 330))
        {
            struct ggml_tensor *dup_node = ggml_dup_tensor(ctx, node);
            dup_node->id = node->id;
            dup_node->op = node->op;
            dup_node->src0 = node->src0;
            dup_node->src1 = node->src1;
            dup_node->perf_runs = node->perf_runs;
            dup_node->perf_cycles = node->perf_cycles;
            dup_node->perf_time_us = node->perf_time_us;
            dup_node->data = malloc(node->nb[3]);
            memcpy(dup_node->data, node->data, node->nb[3]);
            tensor_array->data[tensor_array->size] = dup_node;
            tensor_array->size++;
        }
    }
